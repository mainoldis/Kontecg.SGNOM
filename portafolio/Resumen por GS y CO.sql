SELECT Resumen.[Group], ISNULL([C], 0) AS C, ISNULL([T],0) AS T, ISNULL([A],0) AS A, ISNULL([S],0) AS S, ISNULL([O],0) AS O, ISNULL([C], 0) + ISNULL([T],0) + ISNULL([A],0) + ISNULL([S],0) + ISNULL([O],0) AS Total FROM
(SELECT REL_OccupationalCategories.Code AS Category, SAL_ComplexityGroups.[Group], REL_Templates.Approved
