...
{
    [KontecgAuthorize(PermissionNames.AdministrationLanguages)]
    public class LanguageAppService : KontecgAppServiceBase, ILanguageAppService
    {
        ...
        {
            var languages =
                (await _applicationLanguageManager.GetLanguagesAsync(KontecgSession.CompanyId)).OrderBy(l =>
                    l.DisplayName);
            var defaultLanguage =
        ...
        [KontecgAuthorize(PermissionNames.AdministrationLanguagesCreate,
            PermissionNames.AdministrationLanguagesEdit)]
        public async Task<GetLanguageForEditOutput> GetLanguageForEdit(NullableIdDto input)
        {
            ...
            output.Flags = FlagsHelper
                .FlagNames
                .OrderBy(f => f)
                .Select(f => new ComboboxItemDto(f, FlagsHelper.GetCountryCode(f))
        ...
        public async Task DeleteLanguage(EntityDto input)
        {
            ...
            await _applicationLanguageManager.RemoveAsync(KontecgSession.CompanyId, language.Name);
        }
        ...
        [KontecgAuthorize(PermissionNames.AdministrationLanguagesChangeTexts)]
        public async Task<PagedResultDto<LanguageTextListDto>> GetLanguageTexts(GetLanguageTextsInput input)
        {
            ...
            if (input.BaseLanguageName.IsNullOrEmpty())
            {
                ...
                {
                    defaultLanguage = (await _applicationLanguageManager.GetLanguagesAsync(KontecgSession.CompanyId))
                        .FirstOrDefault();
            ...
            var baseValues = _applicationLanguageTextManager.GetStringsOrNull(KontecgSession.CompanyId, source.Name,
                baseCulture, allStrings.Select(x => x.Name).ToList());
            var targetValues = _applicationLanguageTextManager.GetStringsOrNull(KontecgSession.CompanyId, source.Name,
        ...
        public async Task UpdateLanguageText(UpdateLanguageTextInput input)
        {
            ...
            var source = LocalizationManager.GetSource(input.SourceName);
            await _applicationLanguageTextManager.UpdateStringAsync(KontecgSession.CompanyId, source.Name, culture,
        ...
        {
            if (KontecgSession.MultiCompanySide != MultiCompanySides.Host)
                throw new UserFriendlyException(L("CompaniesCannotCreateLanguage"));
        ...
        [KontecgAuthorize(PermissionNames.AdministrationLanguagesEdit)]
        protected virtual async Task UpdateLanguageAsync(CreateOrUpdateLanguageInput input)
        {
            ...
            await _applicationLanguageManager.UpdateAsync(KontecgSession.CompanyId, language);
        }

        private async Task CheckLanguageIfAlreadyExists(string languageName, int? expectedId = null)
        {
            var existingLanguage = (await _applicationLanguageManager.GetLanguagesAsync(KontecgSession.CompanyId))
