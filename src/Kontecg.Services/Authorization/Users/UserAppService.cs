...
using Kontecg.Authorization.Permissions;
...
{
    [KontecgAuthorize(PermissionNames.AdministrationUsers)]
    public class UserAppService : KontecgAppServiceBase, IUserAppService
    {
        ...
        [KontecgAuthorize(PermissionNames.AdministrationUsersCreate,
            PermissionNames.AdministrationUsersEdit)]
        ...
        [KontecgAuthorize(PermissionNames.AdministrationUsersChangePermissions)]
        ...
        [KontecgAuthorize(PermissionNames.AdministrationUsersDelete)]
        public async Task DeleteUser(EntityDto<long> input)
        {
            if (input.Id == KontecgSession.GetUserId()) throw new UserFriendlyException(L("YouCanNotDeleteOwnAccount"));

            var user = await UserManager.GetUserByIdAsync(input.Id);
            CheckErrors(await UserManager.DeleteAsync(user));
        }

        [KontecgAuthorize(PermissionNames.AdministrationUsersUnlock)]
        ...
        [KontecgAuthorize(PermissionNames.AdministrationUsersEdit)]
        protected virtual async Task UpdateUserAsync(CreateOrUpdateUserInput input)
        {
            ...
            {
                var randomPassword = await _userManager.CreateRandomPasswordAsync();
            ...
            if (input.SendActivationEmail)
            {
                ...
                await _userMailer.SendEmailActivationLinkAsync(
                    user,
                    "", //TODO: Found any form for sending this
                    input.User.Password
                );
            }
        }
        ...
        {
            if (KontecgSession.CompanyId.HasValue) await _userPolicy.CheckUserPolicyAsync(KontecgSession.GetCompanyId());

            var user = ObjectMapper.Map<User>(input.User); //Passwords is not mapped (see mapping configuration)
            user.CompanyId = KontecgSession.CompanyId;
            ...
            foreach (var roleName in input.AssignedRoleNames)
            {
                ...
                user.Roles.Add(new UserRole(KontecgSession.CompanyId, user.Id, role.Id));
            }
            ...
            if (input.SendActivationEmail)
            {
                ...
                await _userMailer.SendEmailActivationLinkAsync(
                    user,
                    null, //TODO: Found any form for sending this
                    input.User.Password
                );
            }
        }
            ...
            if (input.Permissions != null && input.Permissions.Any(p => !p.IsNullOrWhiteSpace()))
            {
                var staticRoleNames = _roleManagementConfig.StaticRoles.Where(
                    r => r.GrantAllPermissionsByDefault &&
                         r.Side == KontecgSession.MultiCompanySide
