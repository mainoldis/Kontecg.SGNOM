...
namespace Kontecg.Currencies.Exchange
{
	/// <summary>
	/// Provides a way to obtain exchange rates to be used in conversion operations
	/// </summary>
	public class TabulatedExchangeRateProvider : IExchangeRateProvider
	{
		...
		public sealed class ExchangeRatePair
		{
			...
			{
				AssertConsistency(direct, inverse);
				Direct = direct;
				Inverse = inverse;
			}

			/// <summary>
			/// Check the consistency of a pair of rates.
			/// </summary>
			...
			public static void AssertConsistency(ExchangeRate direct, ExchangeRate inverse)
			{
				Check.NotNull(direct, nameof(direct));
                Check.NotNull(inverse, nameof(inverse));

				if (direct.From != inverse.To) throw DifferentCurrencyException.ForCodes(direct.From.AlphabeticCode(), inverse.To.AlphabeticCode());
				if (direct.To != inverse.From) throw DifferentCurrencyException.ForCodes(direct.To.AlphabeticCode(), inverse.From.AlphabeticCode());
			}
