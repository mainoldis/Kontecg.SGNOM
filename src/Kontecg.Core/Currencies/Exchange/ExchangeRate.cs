...
namespace Kontecg.Currencies.Exchange
{
	...
	public class ExchangeRate : IEquatable<ExchangeRate>
	{
        ...
        {
            if (rate < 0)
                throw new ArgumentOutOfRangeException(nameof(rate), "non negative");
		...
		/// <summary>
		/// A <see cref="decimal"/> instance representing the relative value of <see cref="From"/> against <see cref="To"/>.
		/// </summary>
		[Pure]
		...
		{
			if (from != From) throw DifferentCurrencyException.ForCodes(From.AlphabeticCode(), from.AlphabeticCode());
		}
		...
		{
			Check.NotNull(rateRepresentation, nameof(rateRepresentation));
